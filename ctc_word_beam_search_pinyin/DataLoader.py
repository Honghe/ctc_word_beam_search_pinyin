...
class DataLoader:
	...
	def __init__(self, dataset, pinyinDict=None, sampleEach=1):
		self.path='./data/'+dataset+'/'
		self.nonWordChars=codecs.open(self.path+'nonWordChars.txt', 'r', 'utf8').read()
		self.lm=LanguageModel(codecs.open(self.path+'corpus.txt', 'r', 'utf8').read(), pinyinDict, self.nonWordChars)
		self.mats=[]
		self.gts=[]
		self.fns=[]
