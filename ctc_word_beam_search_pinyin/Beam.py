...
from .TransformedDict import TransformedDict
...
class Beam:
    ...
    def __init__(self, lm, useNGrams,blankIdx):
    ...
    def isTextEmptyOrAllCtcBlank(self):
        if len(self.textual.text) == 0:
            return True
        if set(self.textual.text) == set([self.blankIdx]):
            return True
        return False
    ...
    def createChildBeam(self, newChar, prBlank, prNonBlank):
        ...
        beam = Beam(self.lm, self.useNGrams, self.blankIdx)
